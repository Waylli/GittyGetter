...
class RepositoriesViewModelSpec: QuickSpec {
    ...
    override class func spec() {
        describe("RepositoriesViewModel") {
            ...
            beforeEach {
                ...
                database = MockDatabase(organizations: organizations, repositories: repositories, fetchedRepositories: fetchedRepositories)
    ...
    private static func makeRepositoriesViewModel(with database: FullRepositoryService,
    ...
    private static func makeErrorThrowingRepositoriesViewModel(with database: FullRepositoryService,
                                                               actions: RepositoriesViewModel.Actions) -> RepositoriesViewModel {
        func errorFunction(query: String, orgs: Organizations, sortingOrder: SortingOrder) -> AnyPublisher<Repositories, CustomError> {
        ...
        let modelInput = RepositoriesViewModel
            .Input(getAllOrganizations: database.getOrganizations,
                   getRepositories: errorFunction(query:orgs:sortingOrder:),
                   updateFavoriteStatus: database.updateFavoriteStatus(of:to:),
                   fetcher: MockFetcher(),
                   configuration: Configuration.standard())
        let modelOutput = RepositoriesViewModel
